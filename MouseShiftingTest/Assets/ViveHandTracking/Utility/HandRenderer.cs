...
class HandRenderer : MonoBehaviour {
  ...
  IEnumerator Start () {
    ...
    targetedController = masterController.gameObject.GetComponent<TargetedController>();           
    ...
    for (int i = 0; i < count; i++) {
      ...
      // JFGA TODO, There must bu only one collider in order to identify the "grabing" action
  ...
  void Update () {
    ...
    //JFGA We change what we need for targeted position
    //transform.position = hand.position;
    transform.position = targetedController.giveRetargetedPosition(hand.position);
    ...
    for (int i = 0; i < points.Count; i++) {
      var go = points[i];
        ...
        go.transform.position = targetedController.giveRetargetedPosition( hand.points[i]);
    ...
    for (int i = 0; i < links.Count; i++) {
      ...
      var pose1 = targetedController.giveRetargetedPosition(hand.points[startIndex]);
      if (!pose1.IsValidGesturePoint())
        continue;

      var pose2 = targetedController.giveRetargetedPosition(hand.points[Connections[i * 2 + 1]]);
